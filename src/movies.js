...
function scoresAverage(moviesArray) {
    return moviesArray.reduce((accum, movie) => {
        let average = accum + movie.score/moviesArray.length;
       let twoDecimals = Math.round(average * 100) / 100
        return twoDecimals
    },0)
}
...
function dramaMoviesScore(moviesArray) {
    let dramaArray = moviesArray.filter(movie => {
        return movie.genre.includes('Drama')
    })
    let scoreSum = dramaArray.reduce((accum, movie) => {
        return accum + movie.score
    },0) 
    let average = scoreSum / dramaArray.length
    return Math.round(average * 100) / 100
}
console.log(dramaMoviesScore(movies))
// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(moviesArray) {
    const movieList = moviesArray.map(movie => movie).sort((a, b) => { 
       return a.year - b.year
    })
    return movieList

}
const newArray = orderByYear(movies);
console.log(newArray);

// Iteration 6: Alphabetic Order - Order by title and print the first 20 titles
function orderAlphabetically(moviesArray) {
    const alphabetisedMovies = moviesArray.map(movie => movie.title).sort((a,b) => a.localeCompare(b))
    return alphabetisedMovies.slice(0,20)
}
...
function turnHoursToMinutes(moviesArray) {
        return moviesArray.map(movie => {
            let hoursToMinute = parseInt(movie.duration) * 60
            let minStrToNum = parseInt(movie.duration.slice(3))
            let sum = hoursToMinute + minStrToNum
